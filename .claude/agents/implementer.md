...
3. The ADRs the spec lists as `Related ADRs`. If the spec doesn't list any, that's a sign the spec is incomplete — flag it before writing code. ADR-0008 (mobile portrait) and ADR-0009 (testing discipline) apply to almost every feature; assume both unless the spec explicitly excludes them.
...
Code that satisfies the spec's acceptance criteria, plus tests written **first** under the red-green-verify loop (ADR-0009).
...
3. **Layer rules from ADR-0004 are absolute.** No game logic in `scenes/`. No state mutation in `ui/`. No Phaser imports in `systems/` without a comment justifying it.
4. **Tile vs pixel (ADR-0005).** Function signatures keep the space explicit. No inline `* TILE_SIZE`.
5. **Seeded RNG only (ADR-0007).** No `Math.random()` in game logic.
6. **Mobile portrait is a real target, no hover dependency (ADR-0008).** Every UI-touching change must work identically on desktop and iPhone Safari portrait. Pointer events only — no `mouse-*` or `touch-*` handlers. Hit areas ≥ 44 × 44 logical px. **No game-relevant information is gated on hover.** Anything a hover would have revealed lives either as an always-visible glyph on the relevant tile/unit, or in the sticky inspection panel for the current selection. Action targeting projects every valid target's cost and hit-chance simultaneously — never one-on-hover.
7. **Red-green-verify (ADR-0009).** Tests come first, not last. The verify step is non-negotiable.
...
- If combat / info design / audio / visuals are touched: the relevant §12 sub-bar still holds, **and the manual play-test was run on iPhone portrait** via the preview URL (ADR-0010).
...
- Preview URL for the iPhone test (from ADR-0010's pipeline), if the change touched UI.
