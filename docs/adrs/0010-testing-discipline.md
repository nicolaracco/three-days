# ADR-0010: Testing discipline (vitest + red-green-verify)
...
| Layer (ADR-0005) | Test type | Tooling |
|---|---|---|
...
| §12 sub-bars (combat feel, info legibility, audio coverage, visual coherence) | **Manual play-test only** | Browser + iPhone (ADR-0009 + ADR-0011) |
...
3. **Verify.** Two checks, both required:
   ...
   - **§12 sub-bar manual play-test** if the change touches combat feel, info design, audio, or visuals. The relevant sub-bar's wording is the test script. Run it on desktop *and* on iPhone portrait via the preview URL (ADR-0011).
...
`pnpm test` runs all vitest tests on every push to a branch with a preview build (ADR-0011). A failing test blocks the preview deploy. This makes "the test suite passes" the precondition for the iPhone test.
...
- Positive: ADR-0005's layer split pays its dividend — most behavior is unit-testable without Phaser, which is fast.
- Positive: The §12 manual play-test list becomes a real document; it's no longer a vibe check.
- Positive: The iPhone test (ADR-0011) becomes the verify step's natural home — testing on the phone is part of every change, not a Day-7 surprise.
