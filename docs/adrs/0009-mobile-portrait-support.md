# ADR-0009: Mobile portrait support
...
The GDD (§4) currently lists "desktop only" as a platform decision. That decision is being reversed: the game must also be playable on iPhone in portrait orientation. Two motivations:

- The developer test loop relies on remote testing from an iPhone (see ADR-0011 for the build pipeline that delivers previews to the phone). The game has to *be* playable on the test device.
- Portability widens the audience for "a stranger plays a 20-minute run" — the GDD §14.2 success criterion.

Phaser 3 unifies mouse and touch into pointer events, so input is mostly free. The non-trivial costs are viewport scaling, HUD layout in portrait aspect, and the **information-design pivot**: GDD §12.2 originally said hovering tiles, enemies, and exits surfaces AP cost, hit chance, threat level, and Day-2 implications. There is no hover on touch, and the right answer is not to fake one — it is to redesign the information layer so hover isn't needed on any device.
...
- Positive: The remote test loop (ADR-0011) is a real test, not a desktop simulation.
...
- Negative: Tile art has to carry more glyph load (HP bar, threat icon, exit type, gate icon, item icon, cover marker). ADR-0007's placeholder atlas needs to be scoped accordingly; Day 7's real-art swap inherits that glyph language.
...
- The placeholder atlas spec (ADR-0007) needs frame-index allocations for the new glyph set (HP bar, threat icons, exit type icons, gate icons, item icons, cover markers). A small spec under `docs/specs/` should capture the atlas layout once Day 1 begins.
